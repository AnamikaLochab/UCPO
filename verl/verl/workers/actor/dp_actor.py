        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    if loss_mode not in ["vanilla", "clip_cov", "kl_cov", "vanilla_ent_adv"]:
                        raise ValueError(f"Unsupported loss mode: {loss_mode}. Supported modes are: 'vanilla', 'clip_cov', 'kl_cov', 'vanilla_ent_adv'.")
                    ...
                    elif loss_mode == "vanilla_ent_adv":
                        advantages += torch.min(0.4 * entropy.detach(), advantages.abs()/2)
                        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_policy_loss(
                            old_log_prob=old_log_prob,
                            log_prob=log_prob,
