...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # )
        advantages, returns, stats = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
        ...
        advantages, returns, stats = core_algos.compute_advantage_pass_K(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            old_log_prob=data.batch["old_log_probs"],
            index=data.non_tensor_batch["uid"],
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.Fgrpo:
        advantages, returns, stats = core_algos.compute_fgrpo_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            old_log_prob=data.batch["old_log_probs"],
            index=data.non_tensor_batch["uid"],
        ...
        advantages, returns, stats = core_algos.compute_dgrpo_advantage_outcome_A_mod(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            old_log_prob=data.batch["old_log_probs"],
            index=data.non_tensor_batch["uid"],
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.DivGRPO_l:
        advantages, returns, stats = core_algos.compute_dgrpo_advantage_outcome_A_mod_l(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            old_log_prob=data.batch["old_log_probs"],
            index=data.non_tensor_batch["uid"],
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.Inv_q:
        advantages, returns, stats = core_algos.compute_dgrpo_advantage_outcome_invq_l(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            old_log_prob=data.batch["old_log_probs"],
            index=data.non_tensor_batch["uid"],
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.DS:
        advantages, returns, stats = core_algos.compute_dgrpo_advantage_outcome_A_diff_smooth(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            old_log_prob=data.batch["old_log_probs"],
            index=data.non_tensor_batch["uid"],
    ...
    return data, all_error_count, stats
...
class RayPPOTrainer(object):
    ...
    def __init__(
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            ...
            AdvantageEstimator.Fgrpo,
            AdvantageEstimator.COVVAR,
            AdvantageEstimator.DivGRPO,
            AdvantageEstimator.DivGRPO_l,
            AdvantageEstimator.Inv_q,
            AdvantageEstimator.DS,
    ...
    def fit(self):
        ...
        def _stack(xs):
            return torch.stack([x if torch.is_tensor(x) else torch.tensor(x) for x in xs]).float()
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch, all_error_count, stats = compute_advantage(
                            batch,
                    ...
                    if self.config.trainer.save_freq > 0 and (
                        ...
                        with _timer("save_checkpoint", timing_raw):
                            ...
                            #     print(f"[DEBUG] Adding barrier after checkpoint save at step {iter_idx}...")
                            #     dist.barrier()
                            #     import time
                            #     time.sleep(3)  # Give filesystem time to sync
                            #     print(f"[DEBUG] Barrier complete at step {iter_idx}")
                ...
                if len(step_stats["Hq_correct"]) > 0:
                    t = _stack(step_stats["Hq_correct"])
                    log_dict["train/Hq_correct_mean"] = t.mean().item()
                    log_dict["train/Hq_correct_median"] = t.median().item()

                if len(step_stats["mc_correct"]) > 0:
                    t = _stack(step_stats["mc_correct"])
                    log_dict["train/mc_correct_mean"] = t.mean().item()

                if len(step_stats["num_correct"]) > 0:
                    t = _stack(step_stats["num_correct"])
                    log_dict["train/num_correct_mean"] = t.mean().item()

                if len(step_stats["Hq_incorrect"]) > 0:
                    t = _stack(step_stats["Hq_incorrect"])
                    log_dict["train/Hq_incorrect_mean"] = t.mean().item()
                if len(step_stats["adv_delta_correct_mean"]) > 0:
                    t = _stack(step_stats["adv_delta_correct_mean"])
                    log_dict["train/adv_delta_correct_mean"] = t.mean().item()

                if len(step_stats["adv_delta_correct_max"]) > 0:
                    t = _stack(step_stats["adv_delta_correct_max"])
                    log_dict["train/adv_delta_correct_max"] = t.mean().item()

                if len(step_stats["adv_delta_correct_min"]) > 0:
                    t = _stack(step_stats["adv_delta_correct_min"])
                    log_dict["train/adv_delta_correct_min"] = t.mean().item()


                if len(step_stats["adv_delta_incorrect_mean"]) > 0:
                    t = _stack(step_stats["adv_delta_incorrect_mean"])
                    log_dict["train/adv_delta_incorrect_mean"] = t.mean().item()

                if len(step_stats["adv_delta_incorrect_max"]) > 0:
                    t = _stack(step_stats["adv_delta_incorrect_max"])
                    log_dict["train/adv_delta_incorrect_max"] = t.mean().item()
                
                if len(step_stats["adv_delta_incorrect_min"]) > 0:
                    t = _stack(step_stats["adv_delta_incorrect_min"])
                    log_dict["train/adv_delta_incorrect_min"] = t.mean().item()

                if len(step_stats["adv_delta_ratio_correct"]) > 0:
                    t = _stack(step_stats["adv_delta_ratio_correct"])
                    log_dict["train/adv_delta_ratio_correct"] = t.mean().item()

                if len(step_stats["adv_delta_ratio_incorrect"]) > 0:
                    t = _stack(step_stats["adv_delta_ratio_incorrect"])
                    log_dict["train/adv_delta_ratio_incorrect"] = t.mean().item()

                if len(step_stats["adv_original_mean"]) > 0:
                    t = _stack(step_stats["adv_original_mean"])
                    log_dict["train/adv_original_mean"] = t.mean().item()

                if len(step_stats["adv_modified_mean"]) > 0:
                    t = _stack(step_stats["adv_modified_mean"])
                    log_dict["train/adv_modified_mean"] = t.mean().item()

                # ===== OURS =====
                if len(step_stats["adv_delta_correct_mean_ours"]) > 0:
                    t = _stack(step_stats["adv_delta_correct_mean_ours"])
                    log_dict["train/adv_delta_correct_mean_ours"] = t.mean().item()

                if len(step_stats["adv_delta_correct_max_ours"]) > 0:
                    t = _stack(step_stats["adv_delta_correct_max_ours"])
                    log_dict["train/adv_delta_correct_max_ours"] = t.mean().item()

                if len(step_stats["adv_delta_correct_min_ours"]) > 0:
                    t = _stack(step_stats["adv_delta_correct_min_ours"])
                    log_dict["train/adv_delta_correct_min_ours"] = t.mean().item()

                if len(step_stats["adv_delta_incorrect_mean_ours"]) > 0:
                    t = _stack(step_stats["adv_delta_incorrect_mean_ours"])
                    log_dict["train/adv_delta_incorrect_mean_ours"] = t.mean().item()

                if len(step_stats["adv_delta_incorrect_max_ours"]) > 0:
                    t = _stack(step_stats["adv_delta_incorrect_max_ours"])
                    log_dict["train/adv_delta_incorrect_max_ours"] = t.mean().item()
                
                if len(step_stats["adv_delta_incorrect_min_ours"]) > 0:
                    t = _stack(step_stats["adv_delta_incorrect_min_ours"])
                    log_dict["train/adv_delta_incorrect_min_ours"] = t.mean().item()

                if len(step_stats["adv_correct_mass_original"]) > 0:
                    t = _stack(step_stats["adv_correct_mass_original"])
                    log_dict["train/adv_correct_mass_original"] = t.mean().item()

                if len(step_stats["adv_correct_mass_modified"]) > 0:
                    t = _stack(step_stats["adv_correct_mass_modified"])
                    log_dict["train/adv_correct_mass_modified"] = t.mean().item()

                if len(step_stats["invq_weight_max"]) > 0:
                    t = _stack(step_stats["invq_weight_max"])
                    log_dict["train/invq_weight_max"] = t.mean().item()

                if len(step_stats["invq_weight_min"]) > 0:
                    t = _stack(step_stats["invq_weight_min"])
                    log_dict["train/invq_weight_min"] = t.mean().item()




                # ===== DIFF - OURS GAP =====
                if len(step_stats["diff_minus_ours_correct_mean"]) > 0:
                    t = _stack(step_stats["diff_minus_ours_correct_mean"])
                    log_dict["train/diff_minus_ours_correct_mean"] = t.mean().item()

                if len(step_stats["diff_minus_ours_correct_max"]) > 0:
                    t = _stack(step_stats["diff_minus_ours_correct_max"])
                    log_dict["train/diff_minus_ours_correct_max"] = t.mean().item()
                
                if len(step_stats["diff_minus_ours_correct_min"]) > 0:
                    t = _stack(step_stats["diff_minus_ours_correct_min"])
                    log_dict["train/diff_minus_ours_correct_min"] = t.mean().item()

                if len(step_stats["diff_minus_ours_incorrect_mean"]) > 0:
                    t = _stack(step_stats["diff_minus_ours_incorrect_mean"])
                    log_dict["train/diff_minus_ours_incorrect_mean"] = t.mean().item()

                if len(step_stats["diff_minus_ours_incorrect_max"]) > 0:
                    t = _stack(step_stats["diff_minus_ours_incorrect_max"])
                    log_dict["train/diff_minus_ours_incorrect_max"] = t.mean().item()
                
                if len(step_stats["diff_minus_ours_incorrect_min"]) > 0:
                    t = _stack(step_stats["diff_minus_ours_incorrect_min"])
                    log_dict["train/diff_minus_ours_incorrect_min"] = t.mean().item()

                if log_dict:
                    logger.log(data=log_dict, step=self.global_steps)
