...
def main(args):
    ...
    if args.source_filter is not None:
        before = len(data)
        data = [x for x in data if x.get(args.source_key) == args.source_filter]
        after = len(data)
        print(f"[filter] {args.source_key}=={args.source_filter}: {before} -> {after}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source_key", type=str, default="source",
                        help="JSON key that stores dataset/source name")
    parser.add_argument("--source_filter", type=str, default=None,
                        help='Only keep rows where source_key == source_filter (e.g., "AIME")')
